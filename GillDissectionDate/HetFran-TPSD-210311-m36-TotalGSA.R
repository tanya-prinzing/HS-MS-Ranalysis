...
GSA_Raw_df <-read_excel("HetFran-TPSD-210311-m36-GSAData.xlsx", sheet="Filaments", col_names = TRUE)
...
GSALamellae_df <- GSA_df %>%
  ...
  mutate(LamFreqBase = (LamCountBase / LFBaseDistanceMM), # Lamellar frequency at base, middle and tip
  ...
  mutate(AvgLamFreqBinMM = mean(c(LamFreqBase, LamFreqMid, LamFreqTip), na.rm = TRUE), # Average lamellar frequency on the median filament
         AvLamAreaBinMM2 = mean(c(LamAreaBaseMM2, LamAreaMidMM2, LamAreaTipMM2), na.rm = TRUE)) %>% # Average lamellar area
  mutate(LamFreqTotalBin = AvgLamFreqBinMM * (MedFilLengthMM * NumFilaments)) %>%
  mutate(LamAreaTotalBin = LamFreqTotalBin * AvLamAreaBinMM2)
...
# Calculate average bilateral lamellae area 
AvLamAreaMM2 <- ((sum(GSALamellae_df$LamAreaTotalBin))/(sum(GSALamellae_df$LamFreqTotalBin))) * 2
# Calculate average lamellar frequency /mm on one side of the filament
...
# Calculate Gill Surface Area in cm2, multiply AvLamFreq by 2 to get both sides of filament
